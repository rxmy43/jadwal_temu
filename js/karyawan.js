...
const alertMessage = document.querySelector('#myModal .alert');
...
/* Edit */
const formEdit = document.getElementById('form_edit_karyawan');
...
function fetchOneKaryawan(event) {
    let id = event.target.getAttribute('data-id');

    $.ajax({
        url: '/php/get_one_karyawan.php',
        type: 'POST',
        dataType: 'json',
        data: {
            id: id,
...
function renderOneData(response) {
    formEdit.querySelector("input[name='nama']").value = response.nama_karyawan;
    ...
    formEdit.querySelector("input[name='departemen']").value = response.jabatan;
...
formEdit.onsubmit = (e) => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', 'php/edit_karyawan.php', true);
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.LOADING) {
...
/* Hapus */
const formDelete = document.getElementById('form_delete_karyawan');
...
formDelete.onsubmit = (e) => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', 'php/delete_karyawan.php', true);
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.LOADING) {
